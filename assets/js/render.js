...
function renderFooter(){
  document.getElementById('footer').innerHTML=`
    <div style="border-top:1px solid var(--bdr);padding:2rem;text-align:center;position:relative;z-index:1;">
      <p style="font-family:var(--mono);font-size:.7rem;color:var(--txt3);letter-spacing:.08em;">
        © 2025 <span style="color:var(--cyan)">${DATA.name}</span> ·
        <a href="mailto:${DATA.email}" style="color:var(--cyan2)">${DATA.email}</a> ·
        <a href="${DATA.linkedin}" target="_blank" style="color:var(--cyan2)">LinkedIn</a>
      </p>
    </div>`;
}
...
function renderExperience(){
  return(DATA.experience||[]).map(e=>{
    ...
    return`<div class="tl-item">
      ...
      <div class="tl-desc">${e.desc}</div>${p}${tags(e.tags)}
    </div>`;
  }).join('');
}
...
function renderEducation(){
  return(DATA.education||[]).map(e=>`
    <div class="edu-card">
      ...
      <div class="edu-det">${(e.detail||'').replace(/\n/g,'<br>')}</div>
    </div>`).join('');
}
...
function renderProjects(){
  return(DATA.projects||[]).map(p=>`
    <div class="proj-card">
      ...
      <div class="proj-out">▶ ${p.outcome}</div>
      ${tags(p.tags)}
    </div>`).join('');
}
...
function renderPublications(){
  return(DATA.publications||[]).map((p,i)=>`
    <div class="pub-item">
      <div class="pub-idx">0${i+1}</div>
      <div>
        <div class="pub-type">${p.type}</div>
        <div class="pub-title">${p.title}</div>
        <div class="pub-venue">${p.venue}</div>
        <div class="pub-authors">${p.authors}</div>
        ${p.doi?`<a href="${p.doi}" target="_blank" style="font-family:var(--mono);font-size:.65rem;color:var(--cyan2);margin-top:.3rem;display:inline-block;">↗ VIEW PAPER</a>`:''}
      </div>
    </div>`).join('');
}
...
function renderSkills(){
  return(DATA.skillGroups||[]).map(g=>`
    <div class="skill-grp">
      <div class="skill-grp-title">${g.title}</div>
      <ul class="skill-list">${(g.skills||[]).map(s=>`
        <li>${s.name}${s.pct!=null?`<span class="skill-bar-w"><div class="skill-bar" style="width:${s.pct}%"></div></span>`:''}</li>`).join('')}
      </ul>
    </div>`).join('');
}
...
function renderDetails(){
  return(DATA.details||[]).map(d=>`
    <div class="det-row">
      <span class="det-key">${d.key}</span>
      <span class="det-val">${d.link?`<a href="${d.link}" target="_blank">${d.val}</a>`:d.val}</span>
    </div>`).join('');
}
...
function renderPhoto(size=200){
  ...
  return`<div class="photo-frame" style="width:${size}px;height:${size}px;">
    <div class="photo-ph">
      ...
      <span>Add photo<br>via CMS</span>
    </div></div>`;
}
